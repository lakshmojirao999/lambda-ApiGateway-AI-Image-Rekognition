$(document).ready(function () {
    ...
    $('#file-form').fileupload({
        ...
        add: function (e, data) {
          ...
          $.post('https://sm5ngwua72.execute-api.us-east-1.amazonaws.com/test/getsigneds3uploadurl', JSON.stringify({ contentType: data.files[0].type }), function (result) {
        ...
        progressall: function (e, data) {
            ...
            $('#output-url').css('display','block').attr('value',data.resultUrl.substr(data.resultUrl.length-28,data.resultUrl.length));
			$('#lbl_result').replaceWith( "File Name -->" + data.resultUrl.substr(data.resultUrl.length-28,data.resultUrl.length) );
        }
    });
}); 